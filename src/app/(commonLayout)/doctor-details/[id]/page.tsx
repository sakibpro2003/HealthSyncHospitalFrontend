...
export default function DoctorDetailsPage() {
  ...
  if (isLoading) return <div className="w-11/12 mx-auto mt-10 text-center">Loading...</div>;
  if (isError || !doctor) return <div className="w-11/12 mx-auto mt-10 text-center text-red-500">Doctor not found.</div>;
  ...
  return (
    <div>
      ...
      <div className="md:col-span-2 space-y-4">
        ...
        <p className="text-lg text-gray-600">{doctor.specialization} - {doctor.department}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <p><span className="font-semibold">Experience:</span> {doctor.experience || "Not Available"}</p>
          {doctor.availability ? (
            <p>
              <span className="font-semibold">Availability:</span>{" "}
              {doctor.availability.days?.join(", ")}<br />
              ({doctor.availability.from} - {doctor.availability.to})
            </p>
          ) : (
            <p><span className="font-semibold">Availability:</span> Not Available</p>
          )}
        </div>
